...
type ActionKind = "suspend" | "unpublish" | "restore";

export function AdminGigActions({ gigId, status }: Props) {
  ...
  const canSuspend = status === "draft" || status === "published";
  ...
  async function submit(kind: ActionKind) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/gigs/${gigId}/${kind}`, {
        method: "POST",
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          {pending === "suspend" ? "Suspending…" : "Suspend gig"}
        </button>
        <button
          type="button"
          onClick={() => submit("unpublish")}
          disabled={!canUnpublish || pending !== null}
        ...
        >
          {pending === "unpublish" ? "Unpublishing…" : "Unpublish gig"}
        </button>
        <button
          type="button"
        ...
        >
          {pending === "restore" ? "Restoring…" : "Restore gig"}
        </button>
      </div>
